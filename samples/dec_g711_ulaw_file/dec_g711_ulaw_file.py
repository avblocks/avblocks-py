#!/usr/bin/env python3
"""
Decode G.711 μ-law file to WAV.

This sample decodes a G.711 μ-law audio file to a WAV file using AVBlocks.
"""
...
@click.option('-i', '--input', 'input_file', 
              help='Input G.711 μ-law file (WAV with μ-law encoding)',
              type=click.Path(exists=True))
@click.option('-o', '--output', 'output_file',
              help='Output WAV file (PCM)',
              type=click.Path())
def main(input_file: str, output_file: str):
    ...
    if not input_file or not output_file:
        ...
        if not input_file:
            input_file = os.path.join(script_dir, "../assets/aud/8k_8bit_1ch_ulaw.wav")
        if not output_file:
            output_file = os.path.join(script_dir, "../output/dec_g711_ulaw_file/8k_8bit_1ch.wav")
