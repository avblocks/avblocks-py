...
This sample encodes a WAV audio file to Opus format using AVBlocks.
"""
...
def create_output_socket(output_file: str) -> MediaSocket:
    """Create output socket for Opus file."""
    # create stream info to describe the output audio stream
    asi = AudioStreamInfo()
    asi.stream_type = StreamType.Opus
...
def main(input_file: str, output_file: str):
    ...
    if not input_file or not output_file:
        ...
        if not output_file:
            output_file = os.path.join(script_dir, "../../output/enc_opus_file/equinox-48KHz.opus")
