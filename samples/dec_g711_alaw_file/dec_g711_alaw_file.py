#!/usr/bin/env python3
"""
Decode G.711 A-law file to WAV.

This sample decodes a G.711 A-law audio file to a WAV file using AVBlocks.
"""
...
@click.option('-i', '--input', 'input_file', 
              help='Input G.711 A-law file (WAV with A-law encoding)',
...
def main(input_file: str, output_file: str):
    ...
    if not input_file or not output_file:
        ...
        if not input_file:
            input_file = os.path.join(script_dir, "../assets/aud/8k_8bit_1ch_alaw.wav")
        if not output_file:
            output_file = os.path.join(script_dir, "../output/dec_g711_alaw_file/8k_8bit_1ch.wav")
