#!/usr/bin/env python3
"""
Encode YUV file to H.265/HEVC.

This sample encodes a raw YUV video file to H.265/HEVC format using AVBlocks.
"""
...
from avblocks import (
    ...
    VideoStreamInfo, StreamType, StreamSubType, 
...
def create_output_socket(output_file: str) -> MediaSocket:
    """Create output socket for H.265 file."""
    socket = MediaSocket()
    socket.file = output_file
    socket.stream_type = StreamType.H265
    socket.stream_sub_type = StreamSubType.HEVC_Annex_B
    ...
    vsi.stream_type = StreamType.H265
    vsi.stream_sub_type = StreamSubType.HEVC_Annex_B
...
def main(input_file: str, output_file: str, fps: float, frame_size: str, 
    ...
    if not input_file or not output_file:
        ...
        if not output_file:
            output_file = os.path.join(script_dir, "../../output/enc_hevc_file/foreman_qcif.h265")
