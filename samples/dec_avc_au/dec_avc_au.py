#!/usr/bin/env python3
"""
Decode H.264/AVC access units to YUV.

This sample decodes a sequence of H.264 access unit files to raw YUV format.
Each AU file contains a single NAL unit (e.g., au_0000.h264, au_0001.h264, etc.).
"""
...
def configure_transcoder(transcoder: Transcoder, au_file: str, output_file: str, color_format: int) -> bool:
    ...
    in_socket.file = None
    # Note: in_socket.stream = None is not needed in Python
...
def decode_aus(input_dir: str, output_file: str, color_format: int) -> bool:
    """Decode H.264 access units to YUV."""
...
COLOR_FORMATS = {
    ...
    'rgb24': ColorFormat.BGR24,
}


@click.command()
@click.option('-i', '--input', 'input_dir', 
              help='Input directory containing AU files (au_0000.h264, au_0001.h264, ...)',
              type=click.Path(exists=True))
@click.option('-o', '--output', 'output_file',
              help='Output YUV file',
              type=click.Path())
@click.option('-c', '--color', 'color_format',
              help='Output color format (yuv420, yv12, nv12, yuv422, bgra32, rgb24)',
              default='yuv420',
